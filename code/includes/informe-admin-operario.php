...
if (isset($_POST['generarInforme'])) {
    ...
    if (strlen($_POST['fechaInicial']) >= 1 && strlen($_POST['fechaFinal']) >= 1 && $_POST['opUsuario'] >= 1) {
        ...
        $consultaC1 = "SELECT SUM(cantidad_dobletroque), SUM(cantidad_tractomulas), SUM(cuatro_manos) FROM servicio WHERE operario_cedula='$usuario' AND cargador_id=1 AND fecha_inicial >= '$f1' AND fecha_final < DATE_ADD('$f2', INTERVAL 1 DAY)";
        ...
        $consultaC2 = "SELECT SUM(cantidad_dobletroque), SUM(cantidad_tractomulas), SUM(cuatro_manos) FROM servicio WHERE operario_cedula='$usuario' AND cargador_id=2 AND fecha_inicial >= '$f1' AND fecha_final < DATE_ADD('$f2', INTERVAL 1 DAY)";
